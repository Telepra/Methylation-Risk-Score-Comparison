...
# Load or define all EPIC array probe IDs
...
run_perm_test <- function(set1, set2, universe, n_perm = 1000) {
...
for (i in 1:(length(studies)-1)) {
  for (j in (i+1):length(studies)) {
    ...
    set1 <- cca_df$probe_id[cca_df[[s1]] == 1]# set1 is the corresponding probe_id's extracted from the CCA file
    ...
    perm_res <- run_perm_test(set1, set2, all_epic_cpgs)
