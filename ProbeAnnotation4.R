...
annotations <- read_csv(
  ...
  mutate(chr_pos = paste(chr, pos, sep = "_"))  # Add combined chr_pos column
...
annotate_study <- function(df) {
  ...
  df %>%
    ...
    mutate(
      ...
      pos = as.integer(pos),
      chr_pos = as.character(chr_pos)  # Ensure chr_pos is character
    ) %>%
    distinct()
}
...
get_counts <- function(df) {
  ...
  # Count by chr_pos instead of just pos
  chr_pos_counts <- df %>%
    dplyr::count(chr_pos, name = "n", sort = TRUE)
  ...
  list(
    genes = gene_counts,
    chr_pos_counts = chr_pos_counts,  # Changed from position_counts to chr_pos_counts
...
output_list <- imap(annotated_studies, function(df, study_name) {
  ...
  list(
    annotated_data = df,
    gene_counts = counts$genes %||% tibble(UCSC_RefGene_Name = character(), n = integer()),
    chr_pos_counts = counts$chr_pos_counts %||% tibble(chr_pos = character(), n = integer()),  # Changed from position_counts
...
write_xlsx(
  flatten(output_list),
  "C:/Users/denni/OneDrive/Desktop/Clark Lab/annotated_results_final2.xlsx"
)